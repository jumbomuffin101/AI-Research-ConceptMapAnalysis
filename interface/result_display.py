"""Streamlit presentation helpers for compact structured grading results."""
...
def category_rows(group_key: str, section: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        {
            "Category": _label(field),
            "Score": section.get(field, {}).get("score", "-"),
        }
...
def _display_category(group_key: str, section: dict[str, Any]) -> None:
    st.dataframe(
        category_rows(group_key, section),
        hide_index=True,
        use_container_width=True,
    )
    decision = section.get("overall_decision")
    if decision:
        st.markdown(f"**Domain overall decision:** {decision}")
...
def _display_summary_items(title: str, items: Any) -> None:
    st.subheader(title)
    ...
    for item in items:
        if isinstance(item, dict):
            st.write(item.get("description") or json.dumps(item, ensure_ascii=False))
        else:
            st.write(f"- {item}")


def display_result(result: Any) -> None:
    """Render one model's compact grading result."""
    ...
    with left:
        _display_summary_items("Strengths", data.get("strengths"))
    with right:
        _display_summary_items("Areas for improvement", data.get("areas_for_improvement"))
...
def display_failure(result: Any) -> None:
    """Render one model failure without hiding successful peer results."""
    model_name = _model_name(result)
    ...
    if "truncated before the complete JSON" in error_message:
        st.warning("Gemma's grading response was truncated before the complete JSON was returned.")
        ...
        st.warning(
            f"{model_name} did not return usable content. Raw response saved for debugging. "
    ...
    with st.expander("Failure details", expanded=True):
        ...
        st.info(
            f"To retry only this model, choose '{model_name}' in the Model selector and "
