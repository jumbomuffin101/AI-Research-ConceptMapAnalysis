...
def model_debug_lines(model_names: Iterable[str] | None = None) -> list[str]:
    ...
    if model_names is None:
        return [
            ...
            "Llama 4 provider: NVIDIA NIM",
            f"Llama 4 model: {LLAMA4_MODEL}",
        ]
...
def _save_nemotron_trace(
    *,
    ...
    DEBUG_DIR.mkdir(parents=True, exist_ok=True)
    debug_path = DEBUG_DIR / (
        f"{timestamp}_{run_id}_{file_stem}_llama4_trace.json"
    )
    payload = {
        "timestamp": timestamp,
        "provider": "NVIDIA NIM",
...
def _save_llama_raw_debug(
    *,
    ...
    """Save Llama 4 content and generation metadata before JSON parsing."""
    DEBUG_DIR.mkdir(parents=True, exist_ok=True)
    ...
    debug_path = DEBUG_DIR / (
        f"{timestamp}_{run_id}_{file_stem}_llama4_attempt{attempt}_raw.json"
    )
...
def _upload_nvidia_asset(
    ...
    """Upload a large image to NVCF and return its asset ID for Llama 4."""
    api_key = _get_secret("NVIDIA_API_KEY")
    if not api_key:
        raise GradingError("NVIDIA_API_KEY is not configured.")
...
def _require_response_content(response: Any, test_name: str) -> str:
    choices = getattr(response, "choices", None)
    if not choices:
        raise ModelResponseError(f"Llama 4 {test_name} returned no response choices.")
    message = getattr(choices[0], "message", None)
    content = getattr(message, "content", None)
    if not isinstance(content, str) or not content.strip():
        raise ModelResponseError(f"Llama 4 {test_name} returned empty content.")
    return content
...
def _run_nemotron_health_tests(
    ...
    image_content, image_metadata = _prepare_request_image(
        "Llama 4", preflight_prompt, image
    )
    image_payload = {
        "model": LLAMA4_MODEL,
...
def _expand_nemotron_compact(
    ...
    result = build_spring_schema(map_file, LLAMA4_MODEL)
    strengths: list[str] = []
...
def _request_model(
    ...
    try:
        ...
        if model_name == "Llama 4" and perform_health_test:
            for line in model_debug_lines([model_name]):
                print(line)
            if health_debug_prefix is None:
                raise GradingError("Llama 4 health-test debug path is missing.")
            if map_file is None:
                raise GradingError("Llama 4 map filename is missing.")
...
def _render_llama_diagnostic_images(
    ...
    """Render exact 2x and 4x PNG inputs for the Llama 4 vision comparison."""
    try:
        import fitz
    except ImportError as exc:
        raise GradingError(
    ...
    *, pdf_path: Path, grading_prompt: str, debug_prefix: Path
    ...
    def call_step(
        ...
        try:
            ...
            raise ModelResponseError(
                f"Llama 4 vision diagnostic failed during {label}.",
...
Only use visible information."""
    ...
    grading_text, grading_response = call_step(
        ...
        grading_prompt,
        selected["base64"],
        MODEL_CONFIGS["Llama 4"]["max_tokens"],
    )
...
def _enforce_llama_relationship_caps(result: dict[str, Any]) -> int:
    """Cap unsupported high Llama 4 scores without inventing replacement evidence."""
    corrected = 0
    for group, fields in CATEGORY_FIELDS.items():
        section = result.get(group)
        if not isinstance(section, dict):
    ...
    if corrected:
        _append_grading_note(
            result,
            f"Llama 4 calibration capped {corrected} criterion score(s) at 2 because "
...
def _check_llama_discrimination(
    *,
    ...
    history_path = DEBUG_DIR / "llama4_score_profile_history.json"
    try:
    ...
    for prior in history:
        ...
        if evidence_gap >= 6 and mean_score_gap <= 0.35:
            _append_grading_note(
                ...
                "identical Llama 4 score profiles.",
            )
...
def run_evaluation(
    ...
    for model_name in names:
        ...
        try:
            ...
            if model_name == "Llama 4" and image_debug_path is not None:
