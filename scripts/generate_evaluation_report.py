...
MODEL_KEYS = {"Gemma": "gemma", "Qwen 3.6 27B": "qwen_3_6_27b"}
MODEL_TITLES = {"gemma": "Gemma", "qwen_3_6_27b": "Qwen 3.6 27B"}
DOMAIN_FIELDS = {
...
def render_report(maps: dict[str, dict[str, Any]]) -> str:
    ...
    for number, entry in enumerate(entries, start=1):
        gemma, llama = _result(entry, "gemma"), _result(entry, "qwen_3_6_27b")
    ...
    comparable = [entry for entry in entries if _result(entry, "gemma") and _result(entry, "qwen_3_6_27b")]
    agreed = sum(_overall(_result(entry, "gemma")) == _overall(_result(entry, "qwen_3_6_27b")) for entry in comparable)
...
def write_machine_summaries(maps: dict[str, dict[str, Any]]) -> None:
    ...
    for entry in entries:
        ...
        row["overall_agreement"] = bool(_result(entry, "gemma") and _result(entry, "qwen_3_6_27b") and _overall(_result(entry, "gemma")) == _overall(_result(entry, "qwen_3_6_27b")))
        rows.append(row)
