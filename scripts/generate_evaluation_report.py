...
def render_report(maps: dict[str, dict[str, Any]]) -> str:
    ...
    for number, entry in enumerate(entries, start=1):
        ...
        lines += [f"## Map {number}: {entry['map_file']}", "", "### Overall", "",
                  f"Gemma: {_overall(gemma)}", f"Llama 3.2 90B Vision: {_overall(llama)}", ""]
        for domain, criteria in DOMAIN_FIELDS.items():
            lines += [f"### {DOMAIN_TITLES[domain]}", "", "| Criterion | Gemma | Llama 3.2 90B Vision |", "|---|---:|---:|"]
            for criterion in criteria:
                lines.append(f"| {criterion.replace('_', ' ').title()} | {_score(gemma, domain, criterion)} | {_score(llama, domain, criterion)} |")
            lines += ["", f"Domain decision — Gemma: {_domain_decision(gemma, domain)}; Llama 3.2 90B Vision: {_domain_decision(llama, domain)}", ""]
    ...
    lines += ["## Cross-Model Summary", "", "| Map | Gemma Overall | Llama 3.2 90B Vision Overall | Agreed | Gemma Avg Score | Llama 3.2 90B Vision Avg Score |", "|---|---|---|---|---:|---:|"]
