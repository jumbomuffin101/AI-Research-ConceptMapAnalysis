...
MODEL_KEYS = {"Gemma": "gemma", "Nemotron": "nemotron"}
MODEL_TITLES = {"gemma": "Gemma", "nemotron": "Nemotron"}
DOMAIN_FIELDS = {
...
def render_report(maps: dict[str, dict[str, Any]]) -> str:
    ...
    for number, entry in enumerate(entries, start=1):
        gemma, llama = _result(entry, "gemma"), _result(entry, "nemotron")
    ...
    comparable = [entry for entry in entries if _result(entry, "gemma") and _result(entry, "nemotron")]
...
def write_machine_summaries(maps: dict[str, dict[str, Any]]) -> None:
    ...
    for entry in entries:
        ...
        row["overall_agreement"] = bool(_result(entry, "gemma") and _result(entry, "nemotron") and _overall(_result(entry, "gemma")) == _overall(_result(entry, "nemotron")))
        rows.append(row)
