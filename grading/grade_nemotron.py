...
MAPS = [
    {
        ...
        "output": "outputs/gradingV5/grounded_map1_llama4.json",
    },
    {
        "label": "Map 2",
        "map_file": "ConceptMap2.pdf",
        "path": "maps/ConceptMap2.pdf",
        "output": "outputs/gradingV5/grounded_map2_llama4.json",
    },
]
...
def extract_evidence(client, image, debug_prefix):
    ...
    content, reason = _response_content(response)
    if reason:
        raise RuntimeError(f"Llama 4 evidence extraction failed: {reason}")
    evidence = json.loads(clean_json_output(content))
    missing = [field for field in EVIDENCE_FIELDS if field not in evidence]
    if missing:
        raise RuntimeError(
            "Llama 4 evidence extraction is missing fields: " + ", ".join(missing)
        )
...
def expand_compact(data, map_file):
    ...
    for group, (score_key, overall_key, evidence_key, label) in DOMAIN_MAP.items():
        ...
        domain["if_no_explanation"] = (
            f"{label} is marked No because Llama 4 identified insufficient visible evidence for this domain."
            if decision == "No"
            else ""
        )
...
def plain_decision(section, field, notes):
    ...
    notes.append(f"Llama 4 omitted {field}; it defaulted to No.")
    return "No"
...
def parse_plain_grading(text, map_file):
    ...
    for index, group in enumerate(PLAIN_SECTIONS):
        ...
        if decision_match:
            ...
            decision = "No"
            notes.append(f"Llama 4 omitted {group}.overall_decision; it defaulted to No.")
    ...
    for item in MAPS:
        ...
        try:
            ...
            save_llama_raw_debug(
                f"{debug_prefix}_llama4_attempt1_raw.json",
                response,
                result,
                prompt,
                1,
            ...
            try:
                ...
                result, reason = _response_content(response)
                if reason:
                    raise RuntimeError(f"Llama 4 grading retry failed: {reason}")
                save_llama_raw_debug(
                    f"{debug_prefix}_llama4_attempt2_raw.json",
                    response,
                    result,
                    retry_prompt,
                    llama_attempt,
            ...
            if len(visible_terms) >= 5 and _is_all_one_result(parsed_result):
                ...
                save_llama_raw_debug(
                    f"{debug_prefix}_llama4_calibration_raw.json",
                    response,
                    result,
                    calibration_prompt,
                    llama_attempt,
